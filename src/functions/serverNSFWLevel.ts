...
const $serverNSFWLevel: FunctionData = {
    name: "$serverNSFWLevel",
    description: "Gets the server nsfw level of a guild.",
    brackets: true,
    returns: "STRING",
    nullable: true,
    optional: true,
    fields: [
        {
            name: "guildID",
            description: "the guild to check for nsfw level.",
            type: "GUILD",
            required: true,
        },
    ],
    execute: async (d, fn) => {
        if (fn.inside) {
            ...
            return fn.resolve(guild.nsfwLevel);
        }

        if (d.data.message instanceof Message) {
            return fn.resolve(d.data.message?.guild?.nsfwLevel);
            ...
            return fn.resolve(d.data.message.message.guild?.nsfwLevel);
        } else if (d.data.message instanceof Guild) {
            return fn.resolve(d.data.message.nsfwLevel);
...
export default $serverNSFWLevel;
